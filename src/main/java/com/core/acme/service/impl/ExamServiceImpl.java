...
public class ExamServiceImpl implements ExamService {
    ...
    public void updateExam(
            String examId, String studentAns) { // updates everytime student sends an answer......
        // to update exam variables and update scores.  the index of current questions are updated
        ...
        Question currentQuestion =
                questionService.getQuestionByQuestionId(
                        test.getQuestionsInTest()
                                .get(exam.getCurrentQuestionIndex())
                                .get(exam.getCurrentSubQuestionIndex()));
        ...
        if (exam.getQuestionNumber() == test.getQuestionsToBeAttempted()) {
            ...
            if (currentQuestion.getCorrectOpt().equals(studentAns)) {
                if (exam.getCurrentSubQuestionIndex()
                        < questionTree.get(exam.getCurrentQuestionIndex()).size()
                                - 1) // questionTree.get(exam.getCurrentQuestionIndex()).listIterator().hasNext())
                { // exam.getCurrentSubQuestionIndex() <
                    // questionTree.get(exam.getCurrentQuestionIndex()).size() - 1) {
                    exam.setCurrentSubQuestionIndex(exam.getCurrentSubQuestionIndex() + 1);
                    System.out.println(
                            "Q:"
                                    + exam.getCurrentQuestionIndex()
                                    + "subQ:"
                                    + exam.getCurrentSubQuestionIndex());
                    exam.setScore(exam.getScore() + 4);
                    System.out.println("correct Ans | score : " + exam.getScore());
                } else if (exam.getCurrentQuestionIndex()
                        < questionTree.size() - 1) // questionTree.listIterator().hasNext())
                ...
                if (exam.getCurrentQuestionIndex()
                        < questionTree.size() - 1) // questionTree.listIterator().hasNext())
                { // incorrect answer given , go to next question skipping all current sub-question
                    // of Question of [next Question exists]
                    exam.setCurrentSubQuestionIndex(0);
                    exam.setCurrentQuestionIndex(exam.getCurrentQuestionIndex() + 1);
                    System.out.println(
                            "Q:"
                    ...
                    // We are on last Question no more questions exist, and we have given incorrect
                    // answer.
    ...
    //        List<List<String>> questionTree = test.getQuestionsInTest();
    //        Question currentQuestion =
    // acmeService.getQuestionByQuestionId(test.getQuestionsInTest().get(exam.getCurrentQuestionIndex()).get(exam.getCurrentSubQuestionIndex()));
    //
    //        if (currentQuestion.getCorrectOpt().equals(studentAns)) {
    //            if
    // (questionTree.get(exam.getCurrentQuestionIndex()).listIterator().hasNext()){//exam.getCurrentSubQuestionIndex() < questionTree.get(exam.getCurrentQuestionIndex()).size() - 1) {
    //                exam.setCurrentSubQuestionIndex(exam.getCurrentSubQuestionIndex() + 1);
    //                System.out.println("Q:" + exam.getCurrentQuestionIndex() + "subQ:" +
    // exam.getCurrentSubQuestionIndex());
    //
    //            } else if (questionTree.listIterator().hasNext()) {    // if [next Question
    // exists]
    //                exam.setCurrentSubQuestionIndex(0);
    //                exam.setCurrentQuestionIndex(exam.getCurrentQuestionIndex() + 1);
    //                System.out.println("Q:" + exam.getCurrentQuestionIndex() + "subQ:" +
    ...
    //            return true;
    //        }else{
    //            if (questionTree.listIterator().hasNext()) {  // incorrect answer given , go to
    // next question skipping all current sub-question of Question of [next Question exists]
    //                exam.setCurrentSubQuestionIndex(0);
    //                exam.setCurrentQuestionIndex(exam.getCurrentQuestionIndex() + 1);
    //                System.out.println("Q:" + exam.getCurrentQuestionIndex() + "subQ:" +
    // exam.getCurrentSubQuestionIndex());
    //                System.out.println(examRepository.save(exam));
    //            } else {
    //                // We are on last Question no more questions exist, and we have given
    // incorrect answer.
    //                // End Exam and Display Results.
    //                isFinished= true;
    ...
    public QuestionDTO getFirstQuestion(String examId) {
        ...
        return questionService.convertQuestionToDTO(
                questionService.getQuestionByQuestionId(firstQuestionId));
    }

    @Override
    public QuestionDTO getNextQuestion(String examId) {
        // if(isCorrect) correct and incorrect logic has been stated in the check answer and next
        // question index has been decided in checkAnswer method
        // we just have to return the next question using the exam.currentQuestionIndex and
        ...
        return questionService.convertQuestionToDTO(
                questionService.getQuestionByQuestionId(nextQuestionId));
    }
